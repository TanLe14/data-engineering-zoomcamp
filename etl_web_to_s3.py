...
def extract_data_to_s3():
    ...
    def cal_number_row(**kwargs):
        ...
        if number_months==1: 
            ...
            for i in range(1,number_months):
                number_row= kwargs['ti'].xcom_pull(task_ids=f'group1.format_to_parquet__{i}')
                sum+= number_row
        return sum 
    @task
    ...
    with TaskGroup(group_id='group1') as gr1: 
        for month in months.split(" "):
            ...
            source_path=base_path+"/data/source/{dataset_file_gz}".format(dataset_file_gz=dataset_file_gz)
            raw_csv_path=base_path+"/data/raw/csv/{dataset_file_csv}".format(dataset_file_csv=dataset_file_csv)
            raw_parquet_path=base_path+"/data/raw/parquet/{dataset_file_parquet}".format(dataset_file_parquet=dataset_file_parquet)
            source_url= "https://github.com/DataTalksClub/nyc-tlc-data/releases/download/{color}/".format(color=color)+dataset_file_gz
